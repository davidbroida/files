...
// TESTS BUG #2
// don't know how to inlcude a dummy token for  a 404 instead of a 401
describe('GET /users/[username]', function() {
	test("raise a 404 status code if user isn't found", async function() {
		const response = await (await request(app).get('/users/john')).send({
			_token: tokens[user[5]]
		});
